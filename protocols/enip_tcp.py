...
def send_rr_cip(cip_pkt, session=0):
    """
    通过ENIP发送CIP数据报文
    :param cip_pkt:
    :return:
    """
    enip_pkt = ENIP_TCP(session=session)
    enip_pkt /= ENIP_SendRRData(items=[
        ENIP_SendUnitData_Item(type_id=0),
        ENIP_SendUnitData_Item() / cip_pkt
    ])
    return enip_pkt


def send_rr_cm_cip(cippkt):
    """将cip报文封装到一个Connection manager中"""
    cipcm_msg = [cippkt]
    import cip
    cippkt = cip.CIP(path=cip.CIP_Path.make(class_id=6, instance_id=1))
    cippkt /= cip.CIP_ReqConnectionManager(message=cipcm_msg)
    return cippkt


def send_rr_mr_cip(cippkt):
    """将CIP报文封装到MultipleService报文中发送到Messag router"""
    cipcm_msg = [cippkt]
    import cip
    cippkt = cip.CIP(path=cip.CIP_Path(wordsize=2, path=b'\x20\x02\x24\x01'))
    cippkt /= cip.CIP_MultipleServicePacket(packets=cipcm_msg)
    return cippkt


def send_unit_cip(cippkt, session_id=0, enip_connid=0, sequence=0):
    """使用ENIP Unit Data来发送CIP数据报文"""
    enippkt = ENIP_TCP(session=session_id)
    enippkt /= ENIP_SendUnitData(items=[
        ENIP_SendUnitData_Item() / ENIP_ConnectionAddress(connection_id=enip_connid),
        ENIP_SendUnitData_Item() / ENIP_ConnectionPacket(sequence=sequence) / cippkt
    ])
    return enippkt
