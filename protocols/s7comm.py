...
S7_TRANSPORT_SIZE_IN_DATA_ITEMS = {
    ...
    0x04: "BYTE/WORD/DWORD (Data_Read)",     # BYTE/WORD/DWORD access, len is in bits
    0x05: "INTEGER (Data_write)",             # Integer access, len is in bits
...
S7_UD_SUB_FUNCTION_CYCLIC = {
    ...
    0x04: "Unsubscribe (Data_Read)"                  # Unsubscribe (disable) cyclic data
}
...
S7_UD_SUB_FUNCTION_CPU = {
    ...
    0x05: "ALARM_8 indication (Data_write)",
...
S7_JB_FUNCTION = {
    ...
    0x04: "Read Var (Data_Read)",
    0x05: "Write Var (Data_write)",
    0x1a: "Request download (0x1a)",
    0x1b: "Download block (Block_Download)",
    0x1c: "Download ended (0x1c)",
    0x1d: "Start upload (Block_upload)",
    0x1e: "Upload (0x1e)",
...
class S7Header(Packet):
    fields_desc = [
        ...
        S7ParameterField("Parameters", None, guess_s7_parameters_class, length_from=lambda x: x.ParameterLength),
...
class S7GetBlockInfoDataReq(Packet):
    fields_desc = [
        ByteEnumField("ReturnCode", 0xff, S7_RETURN_CODE),
        ByteEnumField("TransportSize", 0x09, S7_TRANSPORT_SIZE_IN_DATA_ITEMS),
        # FieldLenField("DataLength", None, fmt="H", length_of="Blocks"), #不存在 Blocks
        ShortField("DataLength", 0x0001),
        StrFixedLenEnumField("BlockType", "08", enum=S7_BLOCK_TYPE_IN_FILE_NAME, length=2),
...
class S7RequestUploadBlockParameterReq(Packet):
    fields_desc = [
        ...
        BitField("Unused", 1, 6),
        FlagsField("FunctionStatus", 0, 2, S7_FUNCTION_STATUS),
        # StrFixedLenField("Unknown1", "\x10\x10", length=2),
        ShortField("Unknown1",0x0000),
...
class S7ReadDiagdataReq(Packet):
    fields_desc = [
        StrFixedLenField("ParameterHead", "\x00\x01\x12", length=3),
        XByteField("ParameterLength", 0x04),
        XByteField("Unknown", 0x11),
        BitEnumField("Type", 4, 4, S7_UD_PARAMETER_TYPE),
        BitEnumField("FunctionGroup", 5, 4, S7_UD_FUNCTION_GROUP),
        # MultiEnumField("SubFunction", 0x0e, S7_SUB_FUNCTIONS, fmt='B', depends_on=lambda p: p.FunctionGroup),
        ByteField('SubFunction',0x0e),
        XByteField("Seq", 0x00),
        XByteField("DURN", 0x00),
        XByteField("LastUnit", 0x00),
        XShortEnumField("ErrorCode", 0x0000, S7_ERROR_CLASS)
...
class S7PIServiceParameterReq(Packet):
    fields_desc = [
        ByteEnumField("Function", 0x28, S7_JB_FUNCTION),
        StrFixedLenField("Unknown1", "\x00\x00\x00\x00\x00\x00\xfd", length=7),
        FieldLenField("ParameterBlockLength", 0x02, fmt="H", length_of="ParameterBlock", adjust=lambda pkt, x: x),
        ...
        FieldLenField("StringLength", 0x05, fmt="B", length_of="PI", adjust=lambda pkt, x: x),
        StrLenField("PI", "P_PROGRAM", length_from=lambda p: p.StringLength)
    ]
