...
def fuzz(params):
    p1 = TPKT() / COTPCR()
    p2 = TPKT() / COTPDT(EOT=1) / S7Header(ROSCTR="Job", Parameters=S7SetConParameter()) # EOT=1 
    p3 = TPKT() / COTPDT(EOT=1) / S7Header(ROSCTR="Job", Parameters=S7WriteVarParameterReq( # EOT=1 
        Function=0x5,
        ItemCount=RandByte(),
