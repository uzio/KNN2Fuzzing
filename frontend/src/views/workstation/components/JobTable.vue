<template>
  <div class="job-table">
    <el-table :data="data" :height="tableHeight" :default-sort="{order: 'descending', prop: 'time'}">
...
export default {
  ...
  methods: {
    ...
    getPercent(job) {
      ...
      return ((100 * job.completed) / job.case.length).toFixed(0); // 相对于用例数
    },
    enterJob(job) {
      if (job.type == 2) {
        // 2表示漏洞挖掘
        ...
        if (job.status == JobStatus.CREATED) {
          ...
          job.status == JobStatus.ERROR || job.status == JobStatus.VULN
        ) {
          this.$router.push({
