  ...
  <div class="fuzz-container">
    <div class="fuzz-param">
      ...
      <div class="control-item">
        ...
        <el-button v-else-if="isVuln" @click="gotoResult">出现可疑漏洞</el-button>
...
export default {
  ...
  computed: {
    ...
    isVuln() {
      if (
        this.$store.state.fuzz.job &&
        this.$store.state.fuzz.job.status == JobStatus.VULN
      ) {
        return true;
      }
      return false;
  ...
  methods: {
    monitor() {
      ...
      if (job.status == JobStatus.FUZZ_COMPLETE || job.status == JobStatus.ERROR || job.status == JobStatus.VULN) {
        // 当测试出错或在测试完成时停止监视器
        if (this.fuzzTimer)
    ...
    gotoResult() {
      let jobId = this.$route.params.id
      this.$router.push({path:`/workstation/job/result/${jobId}`})  // *修改注脚
    },
    viewErrMsg() {
      let job = this.$store.state.fuzz.job
