...
class ScapyField(BaseField):
    ...
    def _mutate(self):    
        ...
        dataSet = Pre.sc(InitSet().seed).readCSV() #NOTE data set of filtering rules has been imporied TODO get the path of rule base file. Set by params['FILTER']
        # start=time()
        while FF:
            loop += 1
            current_value = str(self._fuzz_packet) #Main method for mutate
            ps = Pre.psp.pkt_split(current_value) #NOTE split of current value
            if Pre.pf.filter(dataSet,ps,'ItemCount') or loop >20: #XXX filtering TODO how to change the params['XXXX'] with demand .Set by params['FILTER_OBJ']
                self._current_value = current_value
                FF=False
        # end=time()
        # cost=end-start
        print'\nfilter loop >>' + str(loop) +'\n'# +' | ' + str(cost) + '\n' #TODO (for test)
