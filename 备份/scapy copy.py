...
class ScapyField(BaseField):
    ...
    def _mutate(self):    
        # XXX overrided on Sept.15th, by Sun Zeen

        # during mutation, all we really do is call str(self.fuzz_packet)
        # as scapy performs mutation each time str() is called...

        FF = True #NOTE filter-Flag
        loop = 0 #NOTE Round limit of filtering
        dataSet = Pre.sc(InitSet().seed).readCSV() #NOTE data set of filtering rules has been imporied
        while FF:
            loop += 1
            current_value = str(self._fuzz_packet) #Main method for mutate
            ps = Pre.psp.pkt_split(current_value) #NOTE split of current value
            if Pre.pf.filter(dataSet,ps,'ItemCount') or loop >125: #XXX filtering TODO how to change the params['XXXX'] with demand .Set by params['FILTER_OBJ']
                self._current_value = current_value
                FF=False
        print'\nfilter loop >>' + str(loop) #+'||'+str(self._mutate.im_self.__dict__['_fuzz_packet']['S7Header'].show())+'\n' #TODO (for test)
