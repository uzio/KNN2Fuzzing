...
INFO = {
    'title':'PLC stop时，模糊 ParameterLength',
    'des':  'PLC stop时，模糊 ParameterLength',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ParameterLength=RandShort(),
                                                        Parameters=S7StopCpuParameterReq(),
                                                        )

    READ_SZL_TEMPLATE = Template(name='PLC stop时，模糊 ParameterLength', fields=[
