...
INFO = {
    'title':'PLC stop时，模糊 Parameter中的Unknown1',
    'des':  'PLC stop时，模糊 Parameter中的Unknown1',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,
                                                        Parameters=S7StopCpuParameterReq(
                                                            Unknown1 = ''.format(RandByte()*RandShort())
                                                        ),
                                                        )

    READ_SZL_TEMPLATE = Template(name='PLC stop时，模糊 Parameter中的Unknown1', fields=[
