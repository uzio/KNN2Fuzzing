...
INFO = {
    'title': 'Data write时，模糊ItemCount字段',
    'des':  'Data write时，功能的ItemCount字段用于指示被写入变量的数量',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="Job",
                                                        RedundancyId=RandShort(),
                                                        Parameters=S7WriteVarParameterReq(
                                                            ItemCount=0xff, Items=[S7WriteVarItemsReq(ItemCount=1,
                                                                                                     TransportSize=0x2)]),
                                                        )/S7WriteVarDataReq(Items=[S7WriteVarDataItemsReq()])

    READ_SZL_TEMPLATE = Template(name='Data write时，模糊 Redundancy Identification(Reserved)', fields=[
