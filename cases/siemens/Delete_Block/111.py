...
INFO = {
    'title':'Delete Block时，模糊 Parameter中的 Unknown1',
    'des':  'Delete Block时，模糊 Parameter中的 Unknown1',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR='Job',
                                                        Parameters=S7DeleteBlockParameterReq(
                                                            Unknown1=''.format(RandInt()*RandShort()*RandByte())
                                                        ),
                                                        )

    READ_SZL_TEMPLATE = Template(name='Delete Block时，模糊 Parameter中的 Unknown1', fields=[
