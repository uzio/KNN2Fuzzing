...
INFO = {
    'title':'Get Block Info时，模糊 Data部分的 BlockNum',
    'des':  'Get Block Info时，模糊 Data部分的 BlockNum',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="UserData",
                                                        ...
                                                        Data=S7GetBlockInfoDataReq(
                                                            BlockNum='{}{}'.format(RandShort(),RandByte())
                                                        )
                                                        )

    READ_SZL_TEMPLATE = Template(name='Get Block Info时，模糊 Data部分的 BlockNum', fields=[
