...
INFO = {
    'title':'List bolcks of Type时，模糊 ROSCTR',
    'des':  'List bolcks of Type时，模糊 ROSCTR',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR=RandByte(),
                                                        Parameters=S7ListBlockParameterReq(),
                                                        Data=S7ListBlockDataReq()
                                                        )

    READ_SZL_TEMPLATE = Template(name='List bolcks of Type时，模糊 ROSCTR', fields=[
