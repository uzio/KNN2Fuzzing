...
INFO = {
    'title':'List bolcks of Type时，模糊 Protocol Id',
    'des':  'List bolcks of Type时，模糊 Protocol Id',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=RandByte(),ROSCTR="UserData",
                                                        Parameters=S7ListBlockParameterReq(),
                                                        Data=S7ListBlockDataReq()
                                                        )

    READ_SZL_TEMPLATE = Template(name='List bolcks of Type时，模糊 Protocol Id', fields=[
