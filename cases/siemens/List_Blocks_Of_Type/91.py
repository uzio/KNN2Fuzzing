...
INFO = {
    'title':'List bolcks of Type时，模糊 Data的 TransportSize 字段',
    'des':  'List bolcks of Type时，模糊 Data的 TransportSize 字段',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="UserData",                                                        DataLength=RandShort(),
                                                        Parameters=S7ListBlockParameterReq(
                                                        ),
                                                        Data=S7ListBlockDataReq(
                                                            TransportSize=RandByte()
                                                        )
                                                        )

    READ_SZL_TEMPLATE = Template(name='List bolcks of Type时，模糊 Data的 TransportSize 字段', fields=[
