...
INFO = {
    'title':'Get Block Info时，模糊 ParameterHead',
    'des':  'Get Block Info时，模糊 ParameterHead',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="UserData",
                                                        Parameters=S7GetBlockInfoParameterReq(
                                                            ParameterHead='{}'.format(RandShort()*RandByte())
                                                        ),
                                                        Data=S7GetBlockInfoDataReq()
                                                        )

    READ_SZL_TEMPLATE = Template(name='Get Block Info时，模糊 ParameterHead', fields=[
