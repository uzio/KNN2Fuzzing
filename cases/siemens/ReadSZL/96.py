...
INFO = {
    'title':'读取系统状态列表，模糊 Parameter中 sequence字段 ',
    'des':  '读取系统状态列表，模糊 Parameter中 sequence字段 ',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="UserData",
                                                        Parameters=S7ReadSZLParameterReq(seq=RandByte()),
                                                        Data=S7ReadSZLDataReq()
                                                        )

    READ_SZL_TEMPLATE = Template(name='读取系统状态列表，模糊 Parameter中 sequence字段', fields=[
