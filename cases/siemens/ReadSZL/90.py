...
INFO = {
    'title':'读取系统状态列表，模糊 PDU reference(Protocol Data Unit) ',
    'des':  '读取系统状态列表，模糊 PDU reference(Protocol Data Unit) ',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="UserData",PDUR=RandShort(),
                                                        Parameters=S7ReadSZLParameterReq(),
                                                        Data=S7ReadSZLDataReq()
                                                        )

    READ_SZL_TEMPLATE = Template(name='读取系统状态列表，模糊 PDU reference(Protocol Data Unit)', fields=[
