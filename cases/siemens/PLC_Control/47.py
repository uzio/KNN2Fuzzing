...
INFO = {
    'title':'PLC Control时，模糊 Parameters 中的 ParameterBlock',
    'des':  'PLC Control时，模糊 Parameters 中的 ParameterBlock',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="Job",                                                        DataLength=RandShort(),
                                                        Parameters=S7PIServiceParameterReq(
                                                            ParameterBlock=''.format(RandShort())
                                                        ),
                                                        )

    READ_SZL_TEMPLATE = Template(name='PLC Control时，模糊 Parameters 中的 ParameterBlock', fields=[
