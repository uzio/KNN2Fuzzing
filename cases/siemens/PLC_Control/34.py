...
INFO = {
    'title':'PLC Control时，模糊 Parameters 中的 Unknown1',
    'des':  'PLC Control时，模糊 Parameters 中的 Unknown1',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="Job",                                                        DataLength=RandShort(),
                                                        Parameters=S7PIServiceParameterReq(
                                                            Unknown1=''.format(RandInt()*RandShort()*RandByte())
                                                        ),
                                                        )

    READ_SZL_TEMPLATE = Template(name='PLC Control时，模糊 Parameters 中的 Unknown1', fields=[
