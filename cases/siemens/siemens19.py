...
INFO = {
    'title':'Data Read时，模糊 Parameter中 Item部分Address',
    'des':  'Data Read时，模糊 Parameter中 Item部分Address',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="Job",
                                                        Parameters=S7ReadVarParameterReq(
                                                            Items=S7ReadVarItemsReq(
                                                                Address=RandByte()*RandShort()
                                                            )),
                                                        )

    READ_SZL_TEMPLATE = Template(name='Data Read时，模糊 Parameter中 Item部分Address', fields=[
