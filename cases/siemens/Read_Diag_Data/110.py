...
INFO = {
    'title':'Read diag data时，模糊 Parameter中的 SubFunction 字段',
    'des':  'Read diag data时，模糊 Parameter中的 SubFunction 字段',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR="UserData",
                                                        Parameters=S7ReadDiagdataReq(
                                                            SubFunction=RandByte()
                                                        ),
                                                        Data=S7ForceDataReq()
                                                        )

    READ_SZL_TEMPLATE = Template(name='Read diag data时，模糊 Parameter中的 SubFunction 字段', fields=[
