...
INFO = {
    'title':'Block download时，模糊 Parameters中的FileNameLength字段',
    'des':  'Block download时，模糊 Parameters中的FileNameLength字段',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR='Job',
                                                        Parameters=S7DownloadEndParameterReq(
                                                            FileNameLength=RandByte(),
                                                            Filename=S7FileNameBlock()
                                                        )
                                                        )

    READ_SZL_TEMPLATE = Template(name='Block download时，模糊 Parameters中的FileNameLength字段', fields=[
