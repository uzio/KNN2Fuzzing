...
INFO = {
    'title':'Block download时，模糊 Parameters中的BlockType字段',
    'des':  'Block download时，模糊 Parameters中的BlockType字段',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR='Job',
                                                        Parameters=S7DownloadEndParameterReq(
                                                            Filename=S7FileNameBlock(
                                                                BlockType=RandTermString(1,'sdsdsdswerercxcgg'),
                                                                BlockNum='00001',
                                                                DstFileSystem='P'
                                                            )
                                                        )
    ...
    READ_SZL_TEMPLATE = Template(name='Block download时，模糊 Parameters中的BlockType字段', fields=[
