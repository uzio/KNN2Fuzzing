...
INFO = {
    'title':'Block Upload时，模糊 Protocol Id',
    'des':  'Block Upload时，模糊 Protocol Id',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=RandByte(),ROSCTR="Job",
                                                        Parameters=S7RequestUploadBlockParameterReq(
                                                            Filename=S7FileNameBlock()
                                                        )
                                                        )

    READ_SZL_TEMPLATE = Template(name='Block Upload时，模糊 Protocol Id', fields=[
