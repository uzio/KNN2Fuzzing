...
INFO = {
    'title':'Block Upload时，模糊 Parameters中的BlockNum字段',
    'des':  'Block Upload时，模糊 Parameters中的BlockNum字段',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32,ROSCTR='Job',
                                                        Parameters=S7RequestUploadBlockParameterReq(
                                                            Filename=S7FileNameBlock(
                                                                BlockNum=RandTermString(2,'sdsdsdswerercxcgg')
                                                            )
                                                        )
                                                        )

    READ_SZL_TEMPLATE = Template(name='Block Upload时，模糊 Parameters中的BlockNum字段', fields=[
