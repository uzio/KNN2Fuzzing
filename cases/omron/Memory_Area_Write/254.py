...
INFO = {
    'title':'Memory Area Write,模糊 Gateway_Count',
    'des':  'Memory Area Write,模糊 Gateway_Count',
    'id':   '2',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = OMRON_Header(Gateway_Count=RandByte(),Command_code=0x0102)/MemoryWriteCommand_Data()
    template = Template(name='Memory Area Write,模糊 Gateway_Count', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
