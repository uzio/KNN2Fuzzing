...
INFO = {
    'title':'hart_IP Pass Through request时，模糊 MessageType',
    'des':  'hart_IP Pass Through request时，模糊 MessageType',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = hart_IP_header(MessageType=RandByte(),MessageID=0x03,MessageLength=0x0011)/hart_IP_PassBody()
    template = Template(name='hart_IP Pass Through request时，模糊 Version', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
