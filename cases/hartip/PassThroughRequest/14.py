...
INFO = {
    'title':'hart_IP Pass Through request时，模糊 SequenceNumber',
    'des':  'hart_IP Pass Through request时，模糊 SequenceNumber',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = hart_IP_header(SequenceNumber=RandShort(),MessageID=0x03,MessageLength=0x0011)/hart_IP_PassBody()
    template = Template(name='hart_IP Pass Through request时，模糊 SequenceNumber', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
