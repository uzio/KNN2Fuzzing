...
INFO = {
    'title':'hart_IP Pass Through request时，模糊 Length',
    'des':  'hart_IP Pass Through request时，模糊 Length',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = hart_IP_header(MessageID=0x03,MessageLength=0x0011)/\
             hart_IP_PassBody(Length=RandByte())
    template = Template(name='hart_IP Pass Through request时，模糊 Length', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
