...
INFO = {
    'title':'hart_IP initiate request时，模糊 SequenceNumber',
    'des':  'hart_IP initiate request时，模糊 SequenceNumber',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = hart_IP_header(SequenceNumber=RandShort())/hart_IP_Body()
    template = Template(name='hart_IP initiate request时，模糊 SequenceNumber', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
