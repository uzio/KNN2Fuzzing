...
INFO = {
    'title': '获取EtherNet IP SendRRData，模糊 session',
    'des':   '获取EtherNet IP SendRRData，模糊 session',
    'id':   '1',
    'type': '1',
    'creator': 'lyp',
...
def fuzz(params):
    Packet = ENIP_TCP(command_id=0x006f,session=RandInt()) / scapy_all.Raw('\x01\x00')
    template = Template(name='获取EtherNet IP SendRRData，模糊 session', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
