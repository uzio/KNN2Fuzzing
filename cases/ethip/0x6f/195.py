...
INFO = {
    'title': '获取EtherNet IP SendRRData，模糊 status',
    'des':   '获取EtherNet IP SendRRData，模糊 status',
    'id':   '1',
    'type': '1',
    'creator': 'lyp',
...
def fuzz(params):
    Packet = ENIP_TCP(command_id=0x006f,status=RandInt()) / scapy_all.Raw('\x01\x00')
    template = Template(name='获取EtherNet SendRRData，模糊 status', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
