...
INFO = {
    'title': '获取EtherNet IP ListIdentity，模糊 options',
    'des':   '获取EtherNet IP ListIdentity，模糊 options',
    'id':   '1',
    'type': '1',
    'creator': 'lyp',
...
def fuzz(params):
    Packet = ENIP_TCP(command_id=0x0063,options=RandInt()) / scapy_all.Raw('\x01\x00')
    template = Template(name='获取EtherNet IP ListIdentity，模糊 options', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
