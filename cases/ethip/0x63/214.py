...
INFO = {
    'title': '获取EtherNet IP ListIdentity，模糊 length',
    'des':   '获取EtherNet IP ListIdentity，模糊 length',
    'id':   '1',
    'type': '1',
    'creator': 'lyp',
...
def fuzz(params):
    Packet = ENIP_TCP(command_id=0x0063,length=RandShort()) / scapy_all.Raw('\x01\x00')
    template = Template(name='获取EtherNet IP ListIdentity，模糊 length', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
