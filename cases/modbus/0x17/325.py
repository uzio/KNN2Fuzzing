...
INFO = {
    'title':'读写多个寄存器,模糊WriteReferenceNumber',
    'des':  '读写多个寄存器,模糊WriteReferenceNumber',
    'id':   '2',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x17, unit_id=0x01) / ReadWriteMultipleRegistersRequest(
        WriteReferenceNumber=RandShort())
    template = Template(name='读写多个寄存器,模糊WriteReferenceNumber', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
