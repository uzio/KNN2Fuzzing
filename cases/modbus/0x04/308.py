...
INFO = {
    'title':'读输入寄存器时，寄存器数量的长度延长',
    'des':  '寄存器数量的长度字段为2个字节，延长到4个字节',
    'id':   '5',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    Packet = ModbusHeaderRequest(func_code=0x04, unit_id=0x01) / ReadInputRegistersRequest_v1(
        ReferenceNumber=RandShort(),WordCount=0x0001,ertraField=RandShort())
    template = Template(name='读输入寄存器时，寄存器数量的长度延长', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
