...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x04, unit_id=0x01) / ReadInputRegistersRequest(
        ReferenceNumber=RandShort(),WordCount=RandChoice(0x07D0,0x07D0,0x07ff,0x08D0,0x0AD0,0x0ED0,0xffff))
    template = Template(name='读输入寄存器时,寄存器数量超过地址范围', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
