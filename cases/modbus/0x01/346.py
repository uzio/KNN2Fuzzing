...
INFO = {
    'title':'读线圈时，线圈字段的长度延长',
    'des':  '线圈的长度字段为2个字节，延长到4个字节',
    'id':   '5',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    Packet = ModbusHeaderRequest(func_code=0x01, unit_id=0x01) / ReadCoilsRequest_v1(
        ReferenceNumber=RandShort(),BitCount=0x0001,ertraField=RandShort())
    template = Template(name='读线圈时，线圈字段的长度延长', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
