...
INFO = {
    'title':'写多个保持寄存器时，模糊写入寄存器的值的字节数',
    'des':  '写多个保持寄存器时，ByteCount随机化',
    'id':   '2',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x10, unit_id=0x01) / WriteMultipleRegistersRequest(
        ReferenceNumber=RandShort(),ByteCount=RandByte())
    template = Template(name='写多个保持寄存器时，模糊写入寄存器的值的字节数', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
