...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=Random, unit_id=0x01) / WriteMultipleRegistersRequest(
        ReferenceNumber=RandShort())
    template = Template(name='写多个保持寄存器时，模糊写入寄存器的值', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
