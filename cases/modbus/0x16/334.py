...
INFO = {
    'title':'屏蔽写寄存器，模糊化AndMask',
    'des':  '屏蔽写寄存器，模糊化AndMask',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x16, unit_id=0x01) /MaskWriteRegisterRequest(AndMask=RandShort())

    template = Template(name='屏蔽写寄存器，模糊化AndMask', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
