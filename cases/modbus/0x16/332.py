...
INFO = {
    'title':'屏蔽写寄存器，模糊化OrMask',
    'des':  '屏蔽写寄存器，模糊化OrMask',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x16, unit_id=0x01) /MaskWriteRegisterRequest(OrMask=RandShort())

    template = Template(name='屏蔽写寄存器，模糊化OrMask', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
