...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=Random, unit_id=0x01) / ReadHoldingRegistersRequest(
        ReferenceNumber=RandShort(),WordCount=RandShort())
    template = Template(name='读保持寄存器时功能码错误', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
