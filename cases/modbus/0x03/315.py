...
def fuzz(params):
    Packet = ModbusHeaderRequest(func_code=0x03, unit_id=0x01) / ReadDiscreteInputsRequest_v1(
        ReferenceNumber=RandShort(),BitCount=0x0001,ertraField=RandShort())
    template = Template(name='读保持寄存器时，寄存器数量的长度延长', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
