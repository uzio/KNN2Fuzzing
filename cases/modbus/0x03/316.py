...
INFO = {
    'title':'读保持寄存器,寄存器数量超过地址范围',
    'des':  '离散输入状态的地址范围为1-125(0x007D)，随机几个不在此范围内的地址，ReferenceNumber模糊化',
    'id':   '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x03, unit_id=0x01) / ReadHoldingRegistersRequest(
        ReferenceNumber=RandShort(),WordCount=RandChoice(0x07D0,0x07D0,0x07ff,0x08D0,0x0AD0,0x0ED0,0xffff))
    template = Template(name='读保持寄存器,寄存器数量超过地址范围', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
