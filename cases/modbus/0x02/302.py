...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x02, unit_id=0x01) / ReadDiscreteInputsRequest(
        ReferenceNumber=RandShort(),BitCount=RandShort())
    template = Template(name='读离散输入状态时，模糊化线圈数量', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
