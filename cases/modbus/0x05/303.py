...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x05, unit_id=0x01) / WriteSingleCoilRequest(
        ReferenceNumber=RandShort(),Data=RandShort())
    template = Template(name='写单个线圈时，模糊起始地址', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
