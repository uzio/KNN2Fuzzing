...
INFO = {
    'title':'写单个寄存器时，丢弃value字段',
    'des':  '写单个寄存器时，丢弃value字段',
    'id':   '6',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    Packet = ModbusHeaderRequest(func_code=0x10, unit_id=0x01) / WriteSingleRegisterRequest(
        ReferenceNumber=RandShort())
    template = Template(name='写单个寄存器时，丢弃value字段', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
