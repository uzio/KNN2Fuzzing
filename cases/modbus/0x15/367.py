...
INFO = {
    'title':'写文件记录，模糊子请求中的FileNumber',
    'des':  '写文件记录，模糊子请求中的FileNumber',
    'id':   '2',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    Packet = ModbusHeaderRequest(func_code=0x15, unit_id=0x01) / WriteFileRecordRequest(
        Groups=[WriteFileSubRequest(FileNumber=RandShort())])
    template = Template(name='写文件记录，模糊子请求中的FileNumber', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
