...
INFO = {
    'title':'写文件记录时，记录号超出地址范围',
    'des':  '记录号的地址范围是0x0000-0x270F，在此范围之外取值',
    'id':   '2',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    Random = RandChoice(0x2710, 0x2720, 0x2810, 0x3710, 0x4710, 0xffff)
    Packet = ModbusHeaderRequest(func_code=0x15, unit_id=0x01) / WriteFileRecordRequest(WriteFileSubRequest(Offset=Random))
    template = Template(name='写文件记录时，记录号超出地址范围', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
