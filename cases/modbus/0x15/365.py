...
INFO = {
    'title':'写文件记录，模糊子请求中的RefType',
    'des':  '写文件记录，模糊子请求中的RefType',
    'id':   '2',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    Random  = RandChoice(0x00,0x01,0x02,0x03,0x05)
    Packet = ModbusHeaderRequest(func_code=0x15, unit_id=0x01) / WriteFileRecordRequest(
        Groups=[WriteFileSubRequest(RefType=Random)])
    template = Template(name='写文件记录，模糊子请求中的RefType', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
