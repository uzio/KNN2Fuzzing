...
INFO = {
    'title': 'CIP协议forward close请求报文路径长度模糊化',
    'des':  '通过Ethernet/IP的request/response来发送CIP forward close请求，对Request forward close的路径进行模糊',
    'id':   '1',
    'type': '1',
    'creator': 'shaoshuai',
...
def fuzz(params):
    cip_pkt = CIP(service=0x4e, path=CIP_Path(wordsize=2, path=b"\x01\x00"))
    cip_pkt /= CIP_ReqForwardOpen(path_wordsize=3, path=RandString(3))
    Packet = send_rr_cip(cip_pkt)
    template = Template(name='cip forward close request', fields=[
        ScapyField(Packet,
                   name='template',
                   fuzzable=True,
                   seed=RANDSEED,
