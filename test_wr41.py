...
INFO = {
    'title': 'Data write时，模糊Parameter中Item中的SyntaxId ',
    'des': 'Data write时，模糊Parameter中Item中的SyntaxId ',
    'id': '1',
    'type': '1',
    'creator': 'liuyongpan',
...
def fuzz(params):
    ...
    READ_SZL_PACKET = TPKT() / COTPDT(EOT=1) / S7Header(ProtocolId=0x32, ROSCTR="Job",
                                                        Parameters=S7WriteVarParameterReq(
                                                            Items=S7WriteVarItemsReq(SyntaxId=RandByte(),
                                                            ItemCount=RandShort(),
                                                            BlockNum=RandShort())),
                                                        Data=S7WriteVarDataItemsReq()
                                                        )

    READ_SZL_TEMPLATE = Template(name='Data write时，模糊Parameter中Item中的SyntaxId', fields=[
