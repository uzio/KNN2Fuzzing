...
class TcpTarget(ServerTarget):
    ...
    def __init__(self, name, host, port, max_retries=1, timeout=None, logger=None):
        '''
        :param name: name of the target
        :param host: host ip (to send data to) currently unused
        :param port: port to send to
        :param max_retries: maximum connection retries (default: 1)
    ...
    def pre_test(self, test_num):
        ...
        while self.socket is None and retry_count < self.max_retries:
            ...
            try:
                retry_count += 1
                self.logger.info('host>>>%s, port>>>%s'%(self.host,self.port))# 打印当前host/post
    ...
    def post_test(self, test_num):
        ...
        if not self.try_reconnect():  # 尝试在此轮测试结束后连接目标
            self.report.failed("target can't connect")
